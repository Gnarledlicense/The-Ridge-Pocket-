...
- **Status:** Confirmed real, currently just something to live with.
- **The gap:** data lives in each browser's local storage, tied to that specific device. Logging on your phone and logging on your computer are two completely separate, disconnected copies — nothing currently makes them agree.
- **Confirmed in practice:** test data logged on the computer stayed there; real daily data on the phone stayed there — neither showed up on the other device.
- **Realistic paths, cheapest to most involved:**
  1. Pick one device as "the real one" and treat the rest as scratch/testing only — zero build, works today, just a discipline choice.
  2. Manual sync via Export/Import between devices when needed — works today, but is a manual step, not real syncing.
  3. A real backend so every device reads/writes the same source of truth automatically — the actual fix, but the same category of project as the reminders/push-notifications item above: genuine infrastructure, not a quick patch.
- **Not currently a problem** — noted here specifically so it's a deliberate decision later, not a surprise.
